      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Conductimetro</li>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="col-md-12">
          <div class="card card-info">
            <div class="card-header">
              <h3 class="card-title">Verificación Operacional de Equipos de Campo - Agua - Conductimetro</h3>
            </div>
            ...
            {{ Form::open(['url' => route('agua.verificacion-operacional.update', $cadena->Id), 'method' => 'POST', 'class' => ['form-horizontal formAjax'], 'id' => 'formCadenaCusodia', 'novalidate' => true , 'autocomplete' => 'off', 'onkeydown' => 'return event.key != "Enter";' , 'onsubmit' => 'return validateForm();' ]) }}
                ...
                <div class="card-body">
                  ...
                  <input type="hidden" class="form-control" name="id" id="id" value="{{ isset($conductimetro) ? $conductimetro->Id : '0' }}">  
                  ...
                  <div class="form-group row">
                      ...
                      <div class="col-sm-2">
                          <input type="text" class="form-control" name="codigo_equipo" id="codigo_equipo" value="{{ isset($conductimetro) ? $conductimetro->CodigoEquipo : '' }}">
                      </div>
                  </div>

                  <h5 style="font-weight: bold;">Método: SM 2510 - B</h5> 
                    ...
                    <div class="col-sm-6">
                        <div class="table-responsive">
                            <table id="tblAjuste" class="table table-bordered" style="text-align: center;">
                                <thead>
                                    <tr>
                                        <th colspan="5">Patrón</th>
                                    </tr>
                                    <tr>
                                        <th>Marca</th>
                                        <th>Lote</th>
                                        <th>Concentración µS/cm (Teórico)</th>
                                        <th>Constante de celda ( 1/cm)</th>
                                        <th>X</th>
                                    </tr>
                                </thead>
                                <tbody>
                                  @isset($conductimetro)
                                      @foreach ($conductimetro->ajuste as $ajuste)
                                          <tr>
                                            ...
                                            <td><input type="hidden" name="ajuste_valor[]" value="{{ $ajuste->ConcentracionuSTeorico }}" />{{ $ajuste->ConcentracionuSTeorico }}</td>
                                            <td><input type="text" name="ajuste_constante[]" class="form-control" value="{{ $ajuste->ConstanteCelda }}"></td>
                    ...
                    <div class="col-sm-6">
                        <div class="table-responsive">
                            <table id="tblControl" class="table table-bordered" style="text-align: center;">
                                ...
                                <tbody>
                                  @isset($conductimetro)
                                      @foreach ($conductimetro->control as $control)
                                          <tr>
                                            ...
                                            <td><input type="hidden" name="control_valort[]" value="{{ $control->ConcentracionuSTeorico }}" />{{ $control->ConcentracionuSTeorico }}</td>
                                            <td><input type="text" name="control_us[]" class="form-control"  value="{{ $control->ConcentracionuS }}" /></td>
                                            <td><input type="text" name="control_ms[]" class="form-control" value="{{ $control->ConcentracionmS }}" /></td>
  ...
  <script type = "text/javascript">
    ...
    function validateForm() {
      ...
      if (codigo_equipo === '') {
        Swal.fire({
          ...
          text: 'Debe de ingresar el código de equipo.'
        });
        return false;
      }
    ...
    $("#addAjuste").click(function() {
      ...
      while (totalRows >= 0) {
        ...
        var colConstante = '<td><input type="text" class="form-control" name="ajuste_constante[] /"></td>';
        ...
        $('#tblAjuste tbody').append('<tr id="tablerow' + (nFilas + 1) + '">' + colMarca + colLote + colValor + colConstante + colEliminar + '</tr>');
    ...
    $("#addControl").click(function() {
      ...
      while (totalRows >= 0) {
        ...
        var colUs = '<td><input type="text" name="control_us[]" class="form-control" /></td>';
        var colmS = '<td><input type="text" name="control_ms[]" class="form-control" /></td>';
        ...
        $('#tblControl tbody').append('<tr id="tablerow' + (nFilas + 1) + '">' + colMarca + colLote + colValorTeorico + colUs + colmS + colEliminar + '</tr>');
